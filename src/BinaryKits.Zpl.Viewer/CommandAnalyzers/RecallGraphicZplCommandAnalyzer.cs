    ...
    public class RecallGraphicZplCommandAnalyzer : ZplCommandAnalyzerBase
    {
        ...
        public override ZplElementBase Analyze(string zplCommand, VirtualPrinter virtualPrinter, IPrinterStorage printerStorage)
        {
            ...
            if (commandMatch.Success)
            {
                ...
                int magnificationFactorX = commandMatch.Groups[3].Success && !string.IsNullOrEmpty(commandMatch.Groups[3].Value) ? int.Parse(commandMatch.Groups[3].Value) : 1;
                int magnificationFactorY = commandMatch.Groups[4].Success && !string.IsNullOrEmpty(commandMatch.Groups[4].Value) ? int.Parse(commandMatch.Groups[4].Value) : 1;
